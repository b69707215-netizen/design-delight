...
const programs = [
  { mark: "01", title: "Базовий старт", lessons: "8 уроків", price: "3600 грн", text: "Правила, базова стратегія, тактика й перші впевнені партії." },
  { mark: "02", title: "Перший хід", lessons: "12 уроків", price: "5400 грн", text: "Дебюти, позиційна логіка та регулярна практика з наставником." },
  { mark: "03", title: "Впевнений стратег", lessons: "16 уроків", price: "7000 грн", text: "Глибший аналіз дебюту, мідлгейму й техніки закінчень." },
  { mark: "04", title: "Турнірний форсаж", lessons: "16 уроків", price: "7500 грн", text: "Підготовка до турнірів, розбір партій і психологія гри." },
  { mark: "05", title: "Шлях гросмейстера", lessons: "24 уроки", price: "10500 грн", text: "Елітна програма з персональним менторством та аналітикою." },
];

const audiences = [
  { symbol: "♙", title: "Для дітей", text: "Перетворюємо навчання на інтелектуальну пригоду, що розвиває увагу й мислення." },
  { symbol: "♘", title: "Для дорослих", text: "Допомагаємо почати з нуля або повернути форму через системний, спокійний прогрес." },
  { symbol: "♕", title: "Для профі", text: "Даємо інструменти серйозної підготовки: аналіз, репертуар і турнірна дисципліна." },
];
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-royal-border bg-background/85 px-5 py-4 backdrop-blur-xl md:px-10">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6">
        <Link to="/" className="font-display text-xl font-semibold tracking-[0.16em] text-gold md:text-2xl">
          ♔ GROSS ACADEMY
        </Link>
        <nav className="hidden items-center gap-8 text-xs font-semibold uppercase tracking-[0.18em] text-cream md:flex">
          <Link to="/" activeProps={{ className: "text-gold" }} className="transition-colors hover:text-gold">Головна</Link>
          <Link to="/programs" activeProps={{ className: "text-gold" }} className="transition-colors hover:text-gold">Програми</Link>
          <Link to="/about" activeProps={{ className: "text-gold" }} className="transition-colors hover:text-gold">Про нас</Link>
          <Link to="/contact" activeProps={{ className: "text-gold" }} className="transition-colors hover:text-gold">Контакт</Link>
        </nav>
      </div>
    </header>
  );
    ...
    <AcademyLayout>
      <section className="relative flex min-h-[720px] items-center justify-center px-5 py-20 text-center md:px-10">
        ...
        <div className="relative z-10 mx-auto max-w-5xl">
          <p className="mb-5 text-sm font-semibold uppercase tracking-[0.36em] text-gold">Твій шлях до гросмейстерського рівня</p>
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Button asChild variant="royal" size="lg"><Link to="/contact">Записатися на урок</Link></Button>
            <Button asChild variant="royalOutline" size="lg"><Link to="/programs">Подивитись програми</Link></Button>
          </div>
          <AudienceGrid />
        </div>
      </section>
...
function AudienceGrid() {
  return (
    <div className="mx-auto mt-16 grid max-w-5xl gap-5 md:grid-cols-3">
      {audiences.map((item) => (
        <article key={item.title} className="group rounded-xl border border-royal-border bg-royal-surface p-7 text-left shadow-royal transition-all hover:-translate-y-2 hover:border-gold hover:bg-royal-surface-strong">
          <div className="mb-5 text-5xl text-gold transition-transform group-hover:scale-110">{item.symbol}</div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 md:px-10">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {programs.map((program) => (
            <article key={program.title} className="rounded-xl border border-royal-border bg-[image:var(--gradient-surface)] p-8 shadow-royal transition-all hover:-translate-y-2 hover:border-gold hover:shadow-gold">
  ...
  return (
    <AcademyLayout>
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 md:grid-cols-[1.05fr_0.95fr] md:px-10">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.32em] text-gold">Про академію</p>
          <h1 className="mt-5 text-4xl font-semibold leading-tight text-cream md:text-6xl">Ми не вчимо просто пересувати фігури</h1>
          <p className="mt-7 text-lg leading-9 text-warm-muted">Gross Academy — це екосистема інтелектуального розвитку, де шахи стають мовою стратегічного мислення, дисципліни й впевнених рішень.</p>
...
function ContactForm({ title, compact = false }: { title: string; compact?: boolean }) {
  return (
    <form onSubmit={handleSubmit} className="mx-auto mt-12 max-w-2xl rounded-xl border border-royal-border bg-royal-surface-strong p-6 shadow-royal md:p-10">
      ...
      {compact && (
        <label className="mb-5 block text-sm font-semibold text-gold">
          ...
          <select required className="mt-2 h-12 w-full rounded-md border border-royal-border bg-input px-4 text-cream focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            ...
            {programs.map((program) => <option key={program.title}>{program.title} — {program.price}</option>)}
          </select>
        </label>
      )}
      ...
      <Button type="submit" variant="royal" size="lg" className="mt-8 w-full uppercase tracking-[0.18em]">Надіслати</Button>
    </form>
  );
}
...
function FormField({ label, placeholder, type = "text" }: { label: string; placeholder: string; type?: string }) {
  return (
    <label className="block text-sm font-semibold text-gold">
      {label} *
      <input required type={type} placeholder={placeholder} className="mt-2 h-12 w-full rounded-md border border-royal-border bg-input px-4 text-cream placeholder:text-warm-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
    </label>
  );
}
